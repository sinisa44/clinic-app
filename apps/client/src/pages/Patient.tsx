  ...
  return (
    <div style={{ padding: 40, maxWidth: 800, margin: '0 auto' }}>
      ...
      {notifications.length > 0 && (
        <div style={{ border: '1px solid orange', padding: 10, marginBottom: 20, background: '#fff3e0' }}>
          <h4>Notifications:</h4>
      ...
      {events.length === 0 ? (
        ...
        <ul style={{ listStyle: 'none', padding: 0 }}>
          {events.map(ev => (
            <li key={ev._id} style={{ borderBottom: '1px solid #eee', padding: 15 }}>
              <div style={{ fontWeight: 'bold', fontSize: 18 }}>{ev.title}</div>
              <div>
                {/* Prikaz vremena u lokalnoj zoni pacijenta (browser default) */}
                Start: {format(new Date(ev.startTime), 'dd.MM.yyyy HH:mm')} <br/>
